...
Los contenedores se han convertido en el estándar de facto para ejecutar servicios modernos y nativos de la nube, tanto si se proporcionan como servicio a terceros como si se utilizan internamente.

El informe sobre el [estado actual de DevOps](https://services.google.com/fh/files/misc/state-of-devops-2021.pdf) elaborado por Google ha revelado que el 40 % de las empresas encuestadas se encuentran en la categoría alta en lo que respecta a la adopción de prácticas de DevOps, lo que significa que son capaces de desplegar entre una vez a la semana y una vez al mes, que el código pasa de la fase de preparación a la de producción entre un día y una semana, así como el hecho de que son capaces de restaurar los servicios en menos de un día y, por último, que su tasa de fracaso de los cambios se sitúa entre el 16 % y el 30 %. Esta cifra ha aumentado desde el 23% del año pasado, mientras que la categoría media se redujo del 44% al 28%: Todo ello significa que las empresas están intensificando su juego y acelerando la capacidad de entregar software con más velocidad y mejor estabilidad.

Para destacar otro dato, basado en la [Encuesta CNFC del 2020](https://www.cncf.io/wp-content/uploads/2020/11/CNCF_Survey_Report_2020.pdf), el 92% de las empresas están utilizando contenedores en producción, con un aumento del 300% desde 2016.

La seguridad, por otro lado, no vio un aumento significativo en los números. No es de extrañar, entonces, que la seguridad se haya quedado atrás en esta carrera: las empresas se están centrando en desplegar el software rápidamente, a menudo sin importarles si las cosas se rompen, gracias a los sistemas de recuperación rápida y roll-back que tienen en su lugar (la mayoría de las empresas van por el dicho "romper temprano, romper rápido"). Mientras tanto, la ciberseguridad es un juego intrínsecamente proactivo, en el que el análisis y las opciones de diseño necesarias deben realizarse antes de empezar a desarrollar, lo que dificulta el proceso de desarrollo; algo que la mayoría de las empresas no están dispuestas a sacrificar en nombre de una base de código más segura y robusta.

Para cumplir con este requisito, los entusiastas de la ciberseguridad no tardaron en idear formas de llenar este vacío: herramientas para escanear imágenes de contenedores, auditar la configuración del clúster y el acceso a la red... Estos son sólo un puñado de los muchos intentos de inyectar seguridad en el flujo de trabajo de DevOps, tratando de minimizar la fricción y reducir el tiempo de despliegue, al tiempo que se añade seguridad al producto y se equilibra dicha seguridad con la velocidad.

Aunque hay una cantidad increíble de herramientas que tratan de asegurar los servicios basados en contenedores desde diferentes ángulos, sigue faltando una estrategia clara en torno a la aplicación coherente de medidas defensivas para prevenir y detectar comportamientos no deseados en su infraestructura.

No es ningún secreto que los incidentes basados en contenedores van en aumento: basándonos en dos informes, el [Red Hat State of Kubernetes Security 2021 Report](https://www.redhat.com/rhdc/managed-files/cl-state-kubernetes-security-report-ebook-f29117-202106-en_0.pdf) y el [Tripwire State of Container Security Report](https://www.tripwire.com/solutions/devops/tripwire-dimensional-research-state-of-container-security-report-register) (2019), podemos ver claramente las cifras en torno a este fenómeno: el primero informa de que el 94% de los encuestados afirma haber tenido al menos un incidente de seguridad relacionado con contenedores, mientras que en el segundo, el 60% informó de al menos un incidente de seguridad.
...
Hay un montón de marcos centrados en la seguridad, pero el problema es que no siempre es fácil relacionarlos con las últimas tecnologías, en parte porque son de muy alto nivel y no tienen en cuenta los detalles de la implementación, pero también porque los contenedores requieren un enfoque diferente de la seguridad que las infraestructuras tradicionales, en las instalaciones, y la falta de experiencia en esta área está comenzando a ser un problema.

El NIST, el Instituto Nacional de Normas y Tecnología, ha realizado un gran trabajo estableciendo normas en todos los ámbitos de la seguridad de la información: desde la aplicación hasta la gestión de riesgos, pasando por la seguridad física y los controles.
En concreto, lo que haremos en este artículo es mapear el Marco de Seguridad Cibernética (CSF) del NIST en el mundo de los contenedores para dar nuestro primer paso hacia una estrategia de seguridad de contenedores más consistente.
...
Los registros de la API de Kubernetes pueden indicar cuándo se despliega o destruye un nuevo contenedor. Los agentes que exportan registros y métricas del sistema a una ubicación central (es decir, DataDog, fluentd, rsyslog) pueden instalarse en cada contenedor. Estos son sólo dos consejos que deberían tenerse en cuenta a la hora de crear una estrategia de registro exhaustiva.
...
Las políticas relajadas, las cuentas con privilegios ilimitados y el acceso irrestricto a la red (por nombrar sólo algunos) son ejemplos de configuraciones que no protegen adecuadamente sus servicios. Dado que las configuraciones rara vez cambian en tiempo de ejecución (por lo tanto, son inmutables), normalmente se pueden auditar una vez, y las configuraciones erróneas se detectan antes en el proceso de desarrollo y despliegue, dando la oportunidad de aplicar medidas correctivas.
...
Yendo más allá, el informe de Dynatrace nos desglosa los incidentes más comunes que experimentarán las empresas en 2020:
...
Pero escribir una lista de acciones, crear algunas herramientas en torno a ellas y probarlas una sola vez no ayuda realmente: en los escenarios de casos reales, pocas cosas salen como se espera, y una pequeña desviación puede cambiar la situación drásticamente. Por eso es importante establecer pruebas periódicas de los playbooks y del plan de respuesta a incidentes en su conjunto: con la ayuda de simulaciones puede asegurarse de que su empresa está preparada para afrontar cualquier situación de crisis derivada de una brecha de seguridad, posicionando a su equipo por delante de la curva y siendo capaz de manejar el incidente con el menor impacto posible.

Puedes comenzar ahora mismo empezando con algo pequeño: se pueden utilizar evaluaciones de seguridad sencillas con herramientas como kube-bench o kubesploit como parte de la evaluación regular, mientras que herramientas como falco se pueden utilizar para detectar la actividad maliciosa dentro del clúster de kubernetes en tiempo real, trabajando en lugar de o junto con una solución de logging centralizada.

## Recuperación
Un sistema verdaderamente seguro es aquel que es capaz de recuperarse en caso de que se produzcan desastres, como la caída de la región de su proveedor de alojamiento o como consecuencia de un ataque de seguridad distruptivo (por ejemplo, ransomware).
...
No hay ningún consejo especial en este caso: se aplican los mismos conceptos de siempre, y plataformas como kubernetes han hecho un gran negocio en la ejecución de sistemas escalables de alta disponibilidad capaces de resistir picos de carga y cortes.

Las soluciones multi-nube son el camino para lograr una gran resiliencia, y de hecho están obteniendo más seguimiento a la luz de eventos como el reportado anteriormente: la comprensión del valor y la necesidad de construir resiliencia en sus ofertas está empujando las soluciones en el camino correcto para lograrlo.
...
Hacer copias de seguridad con frecuencia y probarlas regularmente garantiza que tus joyas de la corona estén siempre a salvo y se puedan restaurar en caso de necesidad. Asegúrese de restringir y supervisar el acceso, algo muy valioso para la respuesta a incidentes.
...
Por último, pero no por ello menos importante: siga probando. El hecho de que haya funcionado una vez (tal vez sólo cuando lo configuraste) no garantiza que vaya a funcionar de nuevo. Acuéstese con la seguridad de que si mañana ocurre algo malo, su empresa va a poder recuperarse con el menor daño posible y en el menor tiempo posible.
